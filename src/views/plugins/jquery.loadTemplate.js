(function ($) {
			...
			complete: function (data) {
				...
				if (done === todo || errored) {
					if (
						errored &&
						settings &&
						typeof settings.error === "function"
					) {
	...
	function addToQueue(template, selection, data, settings) {
		if (queue[template]) {
			...
			queue[template] = [
				{ data: data, selection: selection, settings: settings },
			];
		}
	}
	...
	function handleTemplateLoadingError(
		template,
		selection,
		data,
		settings,
		...
		if (
			typeof queue[template] !== "undefined" &&
			queue[template].length > 0
		) {
			queue[template] = [];
		}
	}

	function handleTemplateLoadingSuccess(
		$templateContainer,
		template,
		selection,
		data,
	...
	function bindData(template, data, settings) {
		...
		processElements(
			...
			function ($elem, value) {
				$elem.append(
					applyFormatters($elem, value, "content", settings)
				);
			}
		);
		...
		processElements(
			...
			function ($elem, value) {
				$elem.attr(
					"src",
					applyFormatters($elem, value, "src", settings)
				);
			},
			function ($elem) {
				$elem.remove();
		...
		processElements(
			...
			function ($elem, value) {
				$elem.attr(
					"href",
					applyFormatters($elem, value, "href", settings)
				);
			},
			function ($elem) {
				$elem.remove();
		...
		processElements(
			...
			function ($elem, value) {
				$elem.attr(
					"alt",
					applyFormatters($elem, value, "alt", settings)
				);
			}
		);
		...
		processElements(
			...
			function ($elem, value) {
				$elem.attr(
					"title",
					applyFormatters($elem, value, "title", settings)
				);
			}
		);
		...
		processElements(
			...
			function ($elem, value) {
				$elem.addClass(
					applyFormatters($elem, value, "class", settings)
				);
			}
		);
		...
		processElements(
			...
			function ($elem, value) {
				...
				$linkElem.attr(
					"href",
					applyFormatters($elem, value, "link", settings)
				);
		...
		processElements(
			...
			function ($elem, value) {
				...
				$linkElem.attr(
					"href",
					applyFormatters($elem, value, "link-wrap", settings)
				);
				$elem.wrap($linkElem);
			}
		);
	...
	function valueIsAllowedByBindingOptions(
	...
	function getBindingOptions(bindingOptionsContainer, settings) {
		...
		if (
			bindingOptionsContainer instanceof jQuery &&
			bindingOptionsContainer.attr("data-binding-options")
		) {
			bindingOptions = $.parseJSON(
				bindingOptionsContainer.attr("data-binding-options")
			);
			...
			typeof bindingOptionsContainer === "object" &&
			bindingOptionsContainer.hasOwnProperty("bindingOptions")
		) {
	...
	function processAllElements(template, data, settings) {
		$("[data-template-bind]", template).each(function () {
			...
			$(param).each(function () {
				...
				if (this.attribute) {
					if (
						!valueIsAllowedByBindingOptions(this, value, settings)
					) {
						$this.remove();
						return;
					}
					...
					switch (this.attribute) {
						...
						case "innerHTML":
							$this.html(
								applyDataBindFormatters($this, value, this)
							);
							break;
						case "contentAppend":
							$this.append(
						...
						case "options":
							...
							$(value).each(function () {
								...
								$option
									...
									.text(
										applyDataBindFormatters(
											$this,
											this[optionsData.value.content],
											optionsData
										)
									)
									.attr(
										"selected",
										typeof this[
											optionsData.value.selected
										] == undefined
											? false
											: this[optionsData.value.selected]
									)
									.appendTo($this);
							});
							break;
	...
	function applyDataBindFormatters($elem, value, data, settings) {
		if (data.formatter && formatters[data.formatter]) {
			return (function (formatterSettings) {
				return formatters[data.formatter].call(
					$elem,
					value,
					data.formatOptions,
					formatterSettings
	...
	function applyFormatters($elem, value, attr, settings) {
		...
		if (
			formatterTarget === attr ||
			(!formatterTarget && attr === "content")
		) {
			formatter = $elem.attr("data-format");
			if (formatter && typeof formatters[formatter] === "function") {
				var formatOptions = $elem.attr("data-format-options");
				return (function (formatterSettings) {
					return formatters[formatter].call(
						$elem[0],
						value,
						formatOptions,
						$.extend({}, formatterSettings)
					);
				})(settings);
			}
		}
	...
	addTemplateFormatter(
		...
		function (value, options, internalSettings) {
			if (!options) {
				return;
			}
			...
			if (options.parentElement)
				return $("<" + parentElement + "/>").loadTemplate(
					template,
					value,
					internalSettings
				);
